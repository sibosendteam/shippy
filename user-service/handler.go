...
import (
	...
	pb "sibosend-shippy/user-service/proto/auth"
...
func (srv *service) ValidateToken(ctx context.Context, req *pb.Token, res *pb.Token) error {
	...
	token := ""
	if strings.HasPrefix(strings.ToLower(req.Token), "bearer ") {
		token = strings.Trim((req.Token)[7:], "")
	} else {
		token = req.Token
	}
	log.Println("token:" + token)
	// Decode token
	claims, err := srv.tokenService.Decode(token)
